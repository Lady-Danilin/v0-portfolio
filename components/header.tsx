            ...
            <div className="flex items-center space-x-3">
              {socialLinks.map((link) => (
                <Link
                  ...
                  className="w-8 h-8 rounded-full bg-gradient-to-r from-[#0066CC] to-[#FF6600] flex items-center justify-center text-white hover:scale-110 transition-transform"
                >
            ...
            <nav className="px-4 py-4 space-y-2">
              {navigation.map((item) => (
                <button
                  key={item.name}
                  onClick={() => handleNavClick(item.href)}
                  className="block w-full text-left px-3 py-2 text-sm font-medium text-muted-foreground hover:text-[#0066CC] hover:bg-muted rounded-md transition-colors"
                >
                  {item.name}
                </button>
              ))}
